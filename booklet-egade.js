...
function VD(lbl,dur,mp4){vidC++;const rid='vid-'+vidC;if(!mp4)return `<div class="vw vw-soon"><div class="vd-poster vd-soon"><img src="${LOGO}" style="height:40px;object-fit:contain;opacity:.4"><div style="margin-top:10px;font-size:13px;font-weight:600;color:#bbb">${lbl}</div><div style="margin-top:4px;font-size:10px;color:#ccc">${dur}</div><div style="margin-top:10px;font-size:9px;letter-spacing:.14em;text-transform:uppercase;color:#C9A84C;font-weight:600">Próximamente</div></div><div class="vl"><span>Video — ${lbl}</span><span>${dur}</span></div></div>`;return `<div class="vw" data-rid="${rid}"><button class="fs-btn" onclick="fsOpen(this.parentElement,'vid');markViewed(this.parentElement)">&#x26F6;</button><div class="vd-poster" id="vp-${vidC}" onclick="startVD(this,${vidC})"><img src="${LOGO}" style="height:48px;object-fit:contain;opacity:.8"><div style="margin-top:10px;font-size:13px;font-weight:600;color:#333">${lbl}</div><div style="margin-top:4px;font-size:10px;color:#999">${dur}</div><div class="vd-play">&#9654;</div></div><video id="vv-${vidC}" controls preload="metadata" style="width:100%;aspect-ratio:16/9;background:#fff;display:none" onplay="markViewed(this.closest('.vw'))"><source src="${mp4}" type="video/mp4"></video><div class="vl"><span>Video ${vidC} — ${lbl}</span><span>${dur}</span></div></div>`}
function AU(lbl,dur,src){auC++;const rid='au-'+auC;const id=auC;if(!src)return `<div class="ap ap-soon"><audio id="aud-${id}" preload="none"></audio><div class="ap-top"><div class="ap-info"><div class="at2">${lbl}</div><div class="ad2">EGADE — Próximamente</div></div></div><div class="ap-progress"><div class="ap-progress-fill"></div></div><div class="ap-time"><span>0:00</span><span>${dur}</span></div><div class="ap-controls"><button class="ab-sm" disabled>&#9666;&#9666;</button><button class="ab" disabled>&#9654;</button><button class="ab-sm" disabled>&#9656;&#9656;</button></div></div>`;return `<div class="ap" data-rid="${rid}" data-au-id="${id}">
<audio id="aud-${id}" src="${src}" preload="metadata"></audio>
...
function startVD(poster,id){poster.style.display='none';const vid=document.getElementById('vv-'+id);vid.style.display='block';vid.play().catch(()=>{poster.style.display='';vid.style.display='none'})}
function tA(b){playAU(b,+b.closest('.ap').dataset.auId)}
function playAU(b,id){const aud=document.getElementById('aud-'+id);if(!aud||!aud.src)return;
  if(aud.paused){aud.play().then(()=>{b.innerHTML='&#9646;&#9646;'}).catch(()=>{})}else{aud.pause();b.innerHTML='&#9654;'}
